...
func (h *Handlers) llmAbilityCreateHandler(c *gin.Context) {
	...
	if body.ModelMetaID != nil && *body.ModelMetaID > 0 {
		...
		if err := h.db.First(&meta, *body.ModelMetaID).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				response.FailWithCode(c, 400, response.Msg(c, "模型元数据不存在"), nil)
				return
			}
			response.Fail(c, response.Msg(c, "查询失败"), gin.H{"error": err.Error()})
			return
	...
	if model == "" {
		response.FailWithCode(c, 400, response.Msg(c, "请填写 model 或 model_meta_id"), nil)
		return
	}
	var ch models.LLMChannel
	if err := h.db.First(&ch, body.ChannelId).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			response.FailWithCode(c, 400, response.Msg(c, "渠道不存在"), nil)
			return
		}
		response.Fail(c, response.Msg(c, "查询失败"), gin.H{"error": err.Error()})
		return
	...
	response.Success(c, response.Msg(c, "创建成功"), gin.H{"ability": row})
}
...
func (h *Handlers) llmAbilityPatchHandler(c *gin.Context) {
	...
	if group == "" || model == "" || cid <= 0 {
		response.FailWithCode(c, 400, response.Msg(c, "请提供 query: group, model, channel_id"), nil)
		return
	}
	...
	if body.ModelMetaID != nil {
		if *body.ModelMetaID == 0 {
			...
			if strings.TrimSpace(meta.ModelName) != row.Model {
				response.FailWithCode(c, 400, response.Msg(c, "model_meta 的 model_name 与当前能力 model 不一致"), nil)
				return
			}
	...
	response.Success(c, response.Msg(c, "已更新"), gin.H{"ability": row})
}
...
func (h *Handlers) llmAbilitiesSyncChannelHandler(c *gin.Context) {
	...
	if err := models.SyncLLMAbilitiesFromChannel(h.db, &ch); err != nil {
		response.Fail(c, response.Msg(c, "同步失败"), gin.H{"error": err.Error()})
		return
	}
	response.Success(c, response.Msg(c, "已同步"), gin.H{"channel_id": id})
}
