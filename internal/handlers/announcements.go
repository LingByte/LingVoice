...
func (h *Handlers) announcementsListHandler(c *gin.Context) {
	...
	response.SuccessOK(c, gin.H{"list": rows})
}
...
func (h *Handlers) adminAnnouncementCreateHandler(c *gin.Context) {
	...
	response.Success(c, response.Msg(c, "已创建"), gin.H{"announcement": row})
}
...
func (h *Handlers) adminAnnouncementUpdateHandler(c *gin.Context) {
	...
	if err := h.db.Where("id = ?", id).First(&row).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			response.FailWithCode(c, 404, response.Msg(c, "不存在"), nil)
			return
		}
		response.Fail(c, response.Msg(c, "查询失败"), gin.H{"error": err.Error()})
		return
	...
	response.Success(c, response.Msg(c, "已更新"), gin.H{"announcement": row})
}
