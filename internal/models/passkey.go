...
func GetPasskeyByUserID(db *gorm.DB, userID int) (*PasskeyCredential, error) {
	if userID == 0 {
		logger.Error("GetPasskeyByUserID: empty user ID")
	...
	if err := db.Where("user_id = ?", userID).First(&credential).Error; err != nil {
		...
		// 只有真正的数据库错误才记录日志
		logger.Error(fmt.Sprintf("GetPasskeyByUserID: database error for user %d: %v", userID, err))
		return nil, ErrFriendlyPasskeyNotFound
	}
...
func GetPasskeyByCredentialID(db *gorm.DB, credentialID []byte) (*PasskeyCredential, error) {
	if len(credentialID) == 0 {
		logger.Error("GetPasskeyByCredentialID: empty credential ID")
		return nil, ErrFriendlyPasskeyNotFound
	}
	...
	if err := db.Where("credential_id = ?", credIDStr).First(&credential).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			logger.Error(fmt.Sprintf("GetPasskeyByCredentialID: passkey not found for credential ID length %d", len(credentialID)))
			return nil, ErrFriendlyPasskeyNotFound
		}
		logger.Error(fmt.Sprintf("GetPasskeyByCredentialID: database error for credential ID: %v", err))
		return nil, ErrFriendlyPasskeyNotFound
	}
...
func UpsertPasskeyCredential(db *gorm.DB, credential *PasskeyCredential) error {
	if credential == nil {
		logger.Warn("UpsertPasskeyCredential: nil credential provided")
	...
	return db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Unscoped().Where("user_id = ?", credential.UserID).Delete(&PasskeyCredential{}).Error; err != nil {
			logger.Warn(fmt.Sprintf("UpsertPasskeyCredential: failed to delete existing credential for user %d: %v", credential.UserID, err))
			return fmt.Errorf("Passkey 保存失败，请重试")
		}
		if err := tx.Create(credential).Error; err != nil {
			logger.Warn(fmt.Sprintf("UpsertPasskeyCredential: failed to create credential for user %d: %v", credential.UserID, err))
			return fmt.Errorf("Passkey 保存失败，请重试")
		}
		return nil
	})
...
func DeletePasskeyByUserID(db *gorm.DB, userID int) error {
	if userID == 0 {
		logger.Warn("DeletePasskeyByUserID: empty user ID")
		return fmt.Errorf("删除失败，请重试")
	}
	// 使用Unscoped()进行硬删除，避免唯一索引冲突
	if err := db.Unscoped().Where("user_id = ?", userID).Delete(&PasskeyCredential{}).Error; err != nil {
		logger.Warn(fmt.Sprintf("DeletePasskeyByUserID: failed to delete passkey for user %d: %v", userID, err))
		return fmt.Errorf("删除失败，请重试")
	}
	return nil
}
