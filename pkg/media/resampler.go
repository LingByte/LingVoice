...
// Prefer StreamResampler for repeated fixed-rate conversions (ASR uplink, etc.).
...
// StreamResampler keeps a persistent converter for repeated fixed-rate chunks.
// Avoids allocating a new InterpolatingConverter on every 20 ms frame.
//
// The returned slice from Resample is owned by the caller for that call only;
// it must not be retained across a subsequent Resample on the same instance
// if you also hold references into converter internals (Resample returns a
// detached buffer via Samples()).
...
// NewStreamResampler builds a reusable resampler. When rates match, Resample is a no-op.
func NewStreamResampler(inputRate, outputRate int) *StreamResampler {
	if inputRate <= 0 || outputRate <= 0 {
		return &StreamResampler{inRate: inputRate, outRate: outputRate}
	}
	s := &StreamResampler{inRate: inputRate, outRate: outputRate}
	if inputRate != outputRate {
		s.conv = DefaultResampler(inputRate, outputRate)
	}
	return s
}

// InRate returns the configured input sample rate.
func (s *StreamResampler) InRate() int {
	if s == nil {
		return 0
	}
	return s.inRate
...
func (s *StreamResampler) OutRate() int {
	if s == nil {
		return 0
	}
	return s.outRate
...
// Resample converts one PCM16LE mono chunk. Same-rate returns data unchanged (may alias).
func (s *StreamResampler) Resample(data []byte) ([]byte, error) {
	if s == nil || len(data) == 0 {
		return data, nil
	}
	if s.inRate == s.outRate || s.conv == nil {
		return data, nil
	}
	if _, err := s.conv.Write(data); err != nil {
		return nil, err
	}
	out := s.conv.Samples()
	if out == nil {
...
// When the internal buffer is empty (typical one-shot / StreamResampler path),
// the converted slice is taken as-is — avoids an extra append copy.
func (ic *InterpolatingConverter) Write(p []byte) (n int, err error) {
	...
	if len(ic.buffer) == 0 {
		if ic.sourceRate == ic.targetRate {
			// ConvertSamples returned the input alias — must copy to own it.
			ic.buffer = append([]byte(nil), converted...)
		} else {
			ic.buffer = converted
		}
	} else {
