...
func (tr *AsyncTaskRunner[T]) HandlePacket(h MediaHandler, packet MediaPacket) {
	...
	if tr.ConcurrentMode {
		if tr.taskQueue != nil {
			...
			log.With(logger.WithFields(map[string]interface{}{"packet": packet, "runner": tr})...).Warn("taskQueue is nil")
		}
	} else {
...
func (tr *AsyncTaskRunner[T]) executeTask(ctx context.Context, req PacketRequest[T]) {
	...
	if err != nil {
		log.With(logger.WithFields(map[string]interface{}{"handler": req.H, "error": err})...).Error("Task execution error")
