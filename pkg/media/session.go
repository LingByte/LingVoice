...
inputLoop:
	...
	log.With(logger.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": transport})...).Warn("input transport processing ended")
}
...
func (tl *TransportManager) processOutgoing() {
	...
	log.With(logger.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport})...).Info("output transport processing started")
outputLoop:
	for {
		...
		case <-tl.session.ctx.Done():
			log.With(logger.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport})...).Info("output transport processing canceled")
			break outputLoop
		case packet, ok = <-tl.txqueue:
			if !ok || packet == nil {
				log.With(logger.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport})...).Info("output transport queue closed")
				break outputLoop
			}
		}
	...
	log.With(zap.Any("sessionID", tl.session.ID)).Warn("output transport processing ended")
}
...
func (tl *TransportManager) cleanup() {
	...
	log.With(logger.WithFields(map[string]interface{}{"sessionID": tl.session.ID, "transport": tl.transport})...).Info("transport layer cleaned up")
}
...
func (s *MediaSession) AddOutputTransport(tx MediaTransport, filterFuncs ...PacketFilter) *MediaSession {
	...
	log.With(logger.WithFields(map[string]interface{}{"sessionID": s.ID, "transport": tx, "queueSize": queueSize})...).Info("output transport registered")
	tx.Attach(s)
...
func (s *MediaSession) Serve() error {
	...
	defer func() {
		if err := recover(); err != nil {
			log.With(logger.WithFields(map[string]interface{}{"error": err, "stacktrace": string(debug.Stack())})...).Error("session recover err")
			return
		}
		s.Running = false
	...
	if s.MaxSessionDuration > 0 {
		time.AfterFunc(time.Duration(s.MaxSessionDuration)*time.Second, func() {
			log.With(logger.WithFields(map[string]interface{}{"sessionID": s.ID, "timeout": s.MaxSessionDuration})...).Info("session stopped timeout")
...
func (s *MediaSession) CauseError(sender any, err error) {
	...
	if err != nil {
		...
		if strings.Contains(msg, "use of closed network connection") || strings.Contains(msg, "connection is closed") {
			log.With(logger.WithFields(map[string]interface{}{"sessionID": s.ID, "sender": sender, "error": err})...).Debug("cause error ignored (normal close)")
		} else {
			log.With(logger.WithFields(map[string]interface{}{"sessionID": s.ID, "sender": sender, "error": err})...).Error("cause error")
		}
	}
...
func callHandleWithState(s *MediaSession, handle StateChangeHandler, state StateChange) {
	defer func() {
		if r := recover(); r != nil {
			log.With(logger.WithFields(map[string]interface{}{"sessionID": s.ID, "state": state, "error": r, "handle": handle, "stacktrace": string(debug.Stack())})...).Error("state panic")
		}
	}()
	handle(state)
}
...
func callHandleWithMediaData(s *MediaSession, h MediaHandler, handle MediaHandlerFunc, data MediaData) {
	defer func() {
		if r := recover(); r != nil {
			log.With(logger.WithFields(map[string]interface{}{"sessionID": s.ID, "data": data, "error": r, "handle": handle, "stacktrace": string(debug.Stack())})...).Error("handle panic")
		}
	}()
	handle(h, data)
}
...
func CastOption[T any](options map[string]any) (val T) {
	...
	if err != nil {
		log.With(logger.WithFields(map[string]interface{}{"options": options, "target": reflect.TypeOf(val).Name(), "error": err})...).Error("cast option error")
	}
	return
}
