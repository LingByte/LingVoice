...
func handleTTSResponse(c context.Context, resp *http.Response, info *common.RelayInfo, encoding string, w http.ResponseWriter) (usage any, err *types.NewAPIError) {
	...
	for {
		...
		case MsgTypeAudioOnlyServer:
			if len(msg.Payload) > 0 {
				if _, writeErr := w.Write(msg.Payload); writeErr != nil {
					return nil, types.NewErrorWithStatusCode(
