...
func oaiImage2MiniMaxImageRequest(request dto.ImageRequest) MiniMaxImageRequest {
	...
	if raw, ok := request.Extra["prompt_optimizer"]; ok {
		...
		if err := json.Unmarshal(raw, &promptOptimizer); err == nil {
...
func aspectRatioFromImageRequest(request dto.ImageRequest) string {
	if raw, ok := request.Extra["aspect_ratio"]; ok {
		...
		if err := json.Unmarshal(raw, &aspectRatio); err == nil && aspectRatio != "" {
			return aspectRatio
		}
	}
...
func responseMiniMax2OpenAIImage(response *MiniMaxImageResponse, info *common.RelayInfo) (*dto.ImageResponse, error) {
...
func miniMaxImageHandler(c context.Context, resp *http.Response, info *common.RelayInfo, w http.ResponseWriter) (*dto.Usage, *types.NewAPIError) {
	...
	if err := json.Unmarshal(responseBody, &minimaxResponse); err != nil {
