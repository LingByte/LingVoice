...
func handleTTSResponse(c context.Context, resp *http.Response, info *common.RelayInfo, w http.ResponseWriter) (usage any, err *types.NewAPIError) {
	...
	if strings.HasPrefix(minimaxResp.Data.Audio, "http") {
		w.Header().Set("Location", minimaxResp.Data.Audio); w.WriteHeader(http.StatusFound)
	} else {
...
func handleChatCompletionResponse(c context.Context, resp *http.Response, info *common.RelayInfo, w http.ResponseWriter) (usage any, err *types.NewAPIError) {
	body, readErr := io.ReadAll(resp.Body)
	if readErr != nil {
		return nil, types.NewErrorWithStatusCode(
	...
	for key, values := range resp.Header {
		if !service.ShouldCopyUpstreamHeader(key, values) {
			continue
		}
	...
	w.WriteHeader(resp.StatusCode); w.Write(body)
	return nil, nil
}
