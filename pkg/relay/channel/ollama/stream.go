...
func ollamaStreamHandler(c context.Context, info *common.RelayInfo, resp *http.Response, w http.ResponseWriter) (*dto.Usage, *types.NewAPIError) {
	...
	var responseId = helper.GetResponseID("")
	var created = time.Now().Unix()
	var toolCallIndex int
	start := helper.GenerateStartEmptyResponse(responseId, created, model, nil)
	if data, err := json.Marshal(start); err == nil {
		_ = helper.StringData(w, string(data))
	}
	...
	for {
		...
		// emit stop delta
		stop := helper.GenerateStopResponse(responseId, created, model, nil)
		if stop != nil {
...
func ollamaChatHandler(c context.Context, info *common.RelayInfo, resp *http.Response, w http.ResponseWriter) (*dto.Usage, *types.NewAPIError) {
	...
	if len(toolCalls) > 0 {
		if rawToolCalls, err := json.Marshal(toolCalls); err == nil {
	...
	service.IOCopyBytesGracefully(w, resp, out)
	return usage, nil
}
