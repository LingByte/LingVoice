...
type ChannelOtherSettings struct {
	...
	// ChannelPurpose marks primary use: llm | tts | asr | realtime | mixed | censor (empty = llm).
	ChannelPurpose string `json:"channel_purpose,omitempty"`
	// TTSCluster is VolcEngine OpenSpeech cluster (default volcano_tts).
	TTSCluster string `json:"tts_cluster,omitempty"`
	// TTSRegion is speech region for Azure Speech / AWS Polly overrides.
	TTSRegion string `json:"tts_region,omitempty"`
	// TTSGroupID is MiniMax group id when required by the account.
	TTSGroupID string `json:"tts_group_id,omitempty"`
