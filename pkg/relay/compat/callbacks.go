...
import (
	...
	log "github.com/LingByte/LingVoice/pkg/common/logger"
)
...
// Clone returns an independent copy of c, including callback lists and execution options.
...
func recoverModelCallbackPanic(ctx context.Context, stage string, errp *error) {
	recovered := recover()
	if recovered == nil {
		return
	}
	stack := debug.Stack()
	log.ErrorfContext(ctx, callbackPanicLogFmt, stage, recovered, string(stack))
