...
import (
	...
	"github.com/LingByte/LingVoice/pkg/relay"
)
...
// ChatModel is the minimal interface for LLM chat completion used by
// LLMChunker. The relay.Client satisfies this interface via RelayChatModel.
type ChatModel interface {
	Name() string
	Chat(ctx context.Context, req *relay.ChatRequest) (*relay.ChatResponse, error)
}

// RelayChatModel adapts relay.Client to the ChatModel interface.
type RelayChatModel struct {
	client *relay.Client
	name   string
}

// NewRelayChatModel creates a ChatModel from a relay.Client.
// The name parameter sets the model identifier returned by Name().
func NewRelayChatModel(client *relay.Client, name string) *RelayChatModel {
	return &RelayChatModel{client: client, name: name}
}

func (m *RelayChatModel) Name() string { return m.name }

func (m *RelayChatModel) Chat(ctx context.Context, req *relay.ChatRequest) (*relay.ChatResponse, error) {
	if m.client == nil {
		return nil, errors.New("relay client is nil")
	}
...
func NewLLMChunker(cfg *Config) *LLMChunker {
	...
	if cfg != nil && cfg.ChatModel != nil {
		if cm, ok := cfg.ChatModel.(ChatModel); ok {
			chatModel = cm
		}
	}
	return &LLMChunker{
...
func (c *LLMChunker) Chunk(ctx context.Context, text string, opts *ChunkOptions) ([]Chunk, error) {
	...
	raw := strings.TrimSpace(firstContent(resp))
	if raw == "" {
...
// firstContent extracts the first choice's message content from a relay response.
func firstContent(resp *relay.ChatResponse) string {
