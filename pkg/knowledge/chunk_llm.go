...
func (c *LLMChunker) Chunk(ctx context.Context, text string, opts *ChunkOptions) ([]Chunk, error) {
	...
	text = base.CleanText(text, cleanOpts)
	text = strings.TrimSpace(text)
	if text == "" {
		return nil, ErrEmptyText
	}
...
func defaultPreChunkCleanOptions(opts *ChunkOptions) *base.Options {
	if opts != nil && opts.PreChunkClean != nil {
		return opts.PreChunkClean
	}
	return &base.Options{StripMarkdown: true, DedupLines: true}
}
