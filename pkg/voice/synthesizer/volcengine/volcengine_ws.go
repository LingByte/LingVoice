...
type volcengineWSResponse struct {
	...
	ErrorCode                int
	ErrorMessage             string
	Timestamp                base.SentenceTimestamp
}

func (v *volcengineSpeechSynthesisListener) sendStreamRequest(ctx context.Context, opt VolcengineTTSOption, text string) (base.SentenceTimestamp, error) {
	if text == "" {
		return base.SentenceTimestamp{}, nil
	}
	...
	if v.pool != nil {
		conn, dialMs, fromWarm, err = v.pool.Take(ctx)
	} else {
		startDial := time.Now()
	...
	if err != nil {
		return base.SentenceTimestamp{}, err
	}
	defer conn.Close()
