...
func TestNewAliyunTextCensor(t *testing.T) {
	...
	if client.AccessKeyID != base.GetEnv("ALIYUN_ACCESS_KEY_ID") {
		t.Error("AccessKeyID was set incorrectly")
	}
	if client.AccessKeySecret != base.GetEnv("ALIYUN_ACCESS_KEY_SECRET") {
	...
	endpoint := base.GetEnv("ALIYUN_GREEN_ENDPOINT")
	if endpoint == "" {
