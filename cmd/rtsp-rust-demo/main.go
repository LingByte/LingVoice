// Command rtsp-rust-demo 启动 RTSP 接收服务 + Rust 媒体节点，
// 演示通过 RTSP 协议接收推流并转发到 Rust 媒体节点。
//
// RTSP publisher → Go → Rust room 路由 → 其他协议客户端拉取。
// 当前只实现接收端（推流入站）。
//
// 用法：
//
...
// rustHandler 实现 EventHandler，将 RTSP 推流桥接到 Rust 媒体节点
...
func (h *rustHandler) OnEvent(event common.ProtocolEvent) error {
	...
	case common.EventTrackAdded:
		...
		// 在 Rust 注册 track
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		if err := h.bridge.AddTrack(ctx, event.SessionID, "rtsp-ep", *event.Track); err != nil {
	...
	case common.EventAnswered:
		h.log.Info(">> PLAY 开始", zap.String("session", event.SessionID))
...
func main() {
	var (
		addr     = flag.String("addr", ":554", "RTSP 服务监听地址")
	...
	_ = logger.Init(&logger.LogConfig{
		Level:    "debug",
		Filename: "logs/rtsp-rust-demo.log",
		MaxSize:  100,
		MaxAge:   30,
		Daily:    true,
	}, "dev")
	...
	// 创建 RTSP 服务
	config := rtsp.Config{Addr: *addr}

	handler := newRustHandler(log, bridge)
	srv := rtsp.NewServer(config, handler, log)

	// 启动 RTSP 服务
	if err := srv.Start(); err != nil {
		log.Fatal("RTSP 服务启动失败", zap.Error(err))
	}
	...
	fmt.Printf("\n")
	fmt.Printf("╔══════════════════════════════════════════════════════════╗\n")
	fmt.Printf("║  LingVoice RTSP + Rust Media Demo                        ║\n")
	fmt.Printf("╚══════════════════════════════════════════════════════════╝\n")
	fmt.Printf("\n")
	fmt.Printf("  RTSP 端点:   rtsp://localhost%s/live/{streamKey}\n", *addr)
	fmt.Printf("  Rust 媒体:   %s\n", *rustAddr)
	fmt.Printf("  Room:        demo-room\n")
	fmt.Printf("\n")
	fmt.Printf("  媒体链路 (推流):\n")
	fmt.Printf("    FFmpeg/OBS → RTSP ANNOUNCE+SETUP+PLAY → Go 解析 RTP\n")
	fmt.Printf("    → gRPC PushRtp → Rust room 路由 → 其他协议拉取\n")
	fmt.Printf("\n")
	fmt.Printf("  测试方法:\n")
	fmt.Printf("    ffmpeg -re -i test.mp4 -c copy -f rtsp rtsp://localhost:554/live/test\n")
	fmt.Printf("\n")
	fmt.Printf("  按 Ctrl+C 退出\n")
	fmt.Printf("\n")

	log.Info("rtsp-rust-demo 启动", zap.String("addr", *addr), zap.String("rust", *rustAddr))
