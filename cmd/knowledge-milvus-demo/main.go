...
//
//	go run ./cmd/knowledge-milvus-demo \
...
func main() {
	fs := flag.NewFlagSet("knowledge-milvus-demo", flag.ExitOnError)
	milvusAddr := fs.String("milvus", "", "Milvus gRPC address (required)")
	milvusUser := fs.String("milvus-user", "", "Milvus username")
	milvusPass := fs.String("milvus-pass", "", "Milvus password")
	milvusToken := fs.String("milvus-token", "", "Milvus token")
	milvusDB := fs.String("milvus-db", "", "Milvus database")
	namespace := fs.String("namespace", "lingvoice_demo", "Milvus collection")
	...
	if *milvusAddr == "" || *embedBase == "" || *embedKey == "" {
		fmt.Fprintln(os.Stderr, "required: -milvus -embed-base -embed-key")
		os.Exit(2)
	}
	if *llmKey == "" {
		*llmKey = *embedKey
	...
	if strings.ToLower(*embedProvider) == string(embed.ProviderNVIDIA) {
		embedCfg = embed.Config{
	...
	if err := handler.Ping(ctx); err != nil {
		fail(fmt.Errorf("milvus ping: %w", err))
	}
	...
	fmt.Println("query:", *query, "retrieve:", strategy, "backend: milvus")
	for _, m := range msgs {
		if m == nil {
			continue
		}
